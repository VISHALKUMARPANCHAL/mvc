...
class Core_Model_Resource_Collection_Abstract
{
    ...
    public function prepareQuery()
    {
        if ($this->_select['COLUMNS'] == []) {
            ...
            $cols = implode(', ', $this->_select['COLUMNS']);
        }
        $query = sprintf("SELECT %s FROM %s", $cols, $this->_select['FROM']);
        if (isset($this->_select['LEFT_JOIN'])) {
            $joinsql = "";
            foreach ($this->_select['LEFT_JOIN'] as $leftJoin) {
                $joinsql .= sprintf(" LEFT JOIN  %s ON %s ", $leftJoin['tableName'], $leftJoin['condition']);
            }
            $query = $query . " " . $joinsql;
        }
